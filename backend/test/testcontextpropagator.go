...
type testContextPropagator struct {
}

var _ workflow.ContextPropagator = &testContextPropagator{}

// Extract implements workflow.ContextPropagator
func (*testContextPropagator) Extract(ctx context.Context, metadata *workflow.Metadata) (context.Context, error) {
...
func (*testContextPropagator) ExtractToWorkflow(ctx workflow.Context, metadata *workflow.Metadata) (workflow.Context, error) {
	ms := metadata.Get("mydata")
...
func (*testContextPropagator) Inject(ctx context.Context, metadata *workflow.Metadata) error {
	d := myValues(ctx)
...
func (*testContextPropagator) InjectFromWorkflow(ctx workflow.Context, metadata *workflow.Metadata) error {
	d := myValuesWf(ctx)
	b, err := json.Marshal(d)
	if err != nil {
		return err
