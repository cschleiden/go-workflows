...
-- Read instance
local instance = cjson.decode(server.call("GET", instanceKey))
...
for i = 1, executedEvents do
    ...
    server.call("XADD", historyStreamKey, sequenceId, "event", eventData)
...
server.call("XTRIM", pendingEventsKey, "MINID", lastPendingEventMessageId)
server.call("XDEL", pendingEventsKey, lastPendingEventMessageId)
...
if state == ContinuedAsNew or state == Finished then
    ...
    server.call("DEL", activeInstanceExecutionKey)

    instance["completed_at"] = now

    server.call("SREM", activeInstancesKey, instanceSegment)
end
...
server.call("SET", instanceKey, cjson.encode(instance))
...
for i = 1, timersToCancel do
    ...
    local eventRemoved = server.call("ZREM", futureEventZSetKey, futureEventKey)
    ...
    if eventRemoved == 1 then
        -- remove payload
        local eventId = server.call("HGET", futureEventKey, "id")
        server.call("HDEL", payloadHashKey, eventId)
        -- remove event hash
        server.call("DEL", futureEventKey)
    end
end
...
for i = 1, timersToSchedule do
    ...
    server.call("ZADD", futureEventZSetKey, timestamp, futureEventKey)
	server.call("HSET", futureEventKey, "instance", instanceSegment, "id", eventId, "event", eventData, "queue", instance["queue"])
	storePayload(eventId, payloadData)
end
...
for i = 1, activities do
    ...
    server.call("SADD", prefix .. "activities:queues", activitySetKey)

    local added = server.call("SADD", activitySetKey, activityId)
	if added == 1 then
		server.call("XADD", activityStreamKey, "*", "id", activityId, "data", activityData)
	end
end
...
for i = 1, otherWorkflowInstances do
    ...
    if createNewInstance == 1 then
        ...
        local instanceExists = server.call("EXISTS", targetActiveInstanceExecutionKey)
        if instanceExists == 1 then
            server.call("XADD", pendingEventsKey, "*", "event", conflictEventData)
            ...
            -- Create new instance
            server.call("SETNX", targetInstanceKey, targetInstanceState)

            -- Set active execution
            server.call("SET", targetActiveInstanceExecutionKey, targetActiveInstanceExecutionState)

            -- Track active instance
            server.call("SADD", activeInstancesKey, targetInstanceSegment)
            server.call("ZADD", instancesByCreation, nowUnix, targetInstanceSegment)
        end
    end
    ...
    for j = 1, eventsToDeliver do
        ...
        if not skipEvents then
            ...
            server.call("XADD", instancePendingEventsKey, "*", "event", eventData)
    ...
    if not skipEvents then
        ...
        server.call("SADD", workflowQueuesSetKey, instanceQueueSetKey)
        local added = server.call("SADD", instanceQueueSetKey, targetInstanceSegment)
        if added == 1 then
            server.call("XADD", instanceQueueStreamKey, "*", "id", targetInstanceSegment, "data", "")
        end
    end
end
...
local task = server.call("XRANGE", workflowStreamKey, taskId, taskId)
if #task ~= 0 then
    local id = task[1][2][2]
    server.call("SREM", workflowSetKey, id)
    server.call("XACK", workflowStreamKey, groupName, taskId)
    server.call("XDEL", workflowStreamKey, taskId)
end

-- If there are pending events, queue the instance again
local pending_events = server.call("XLEN", pendingEventsKey)
