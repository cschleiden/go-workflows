...
func Test_AutoExpiration(t *testing.T) {
	...
	setup := getCreateBackend(valkeyClient, WithAutoExpiration(autoExpirationTime))
	b := setup()
	...
	// Wait for Valkey to expire the keys
	...
	insts, err := b.(*valkeyBackend).GetWorkflowInstances(ctx, "", "", 1)
	require.NoError(t, err)
...
func Test_AutoExpiration_ContinueAsNew_SubWorkflow(t *testing.T) {
	...
	valkeyClient := getClient()
	setup := getCreateBackend(valkeyClient, WithAutoExpiration(0), WithAutoExpirationContinueAsNew(autoExpirationTime))
	b := setup()
