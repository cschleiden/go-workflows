...
func Test_ExecuteWorkflowWithTimerCancellation(t *testing.T) {
	r := NewRegistry()
	...
	Workflow1 := func(ctx sync.Context) error {
		...
		sc := NewSignalChannel(ctx, "cancelTimer")

		tctx, cancel := sync.WithCancel(ctx)
		timer, err := ScheduleTimer(tctx, time.Millisecond*5)
		if err != nil {
			panic("error executing activity 1")
		}

		s := sync.NewSelector()
		s.AddFuture(timer, func(ctx sync.Context, f sync.Future) {
			err := f.Get(ctx, nil)
			if err != nil {
				// timer was cancelled
			} else {
				require.Fail(t, "timer fired")
			}
		})
		s.AddChannelReceive(sc, func(ctx sync.Context, c sync.Channel) {
			cancel()
		})
		s.Select(ctx)
